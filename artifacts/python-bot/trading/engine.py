...
class TradingEngine:
    def __init__(self, mode: str, strategies: list, sentiment_analyzer, ws_client, mt5_available: bool):
        ...
        self.block_on_ai_error: bool = False  # False = allow trade when all AI providers fail (rate-limited/no credits)
    ...
    async def _handle_config_update(self, config: dict):
        ...
        if "blockOnAiError" in config:
            self.block_on_ai_error = bool(config["blockOnAiError"])
            log.info(f"block_on_ai_error set to {self.block_on_ai_error}")
